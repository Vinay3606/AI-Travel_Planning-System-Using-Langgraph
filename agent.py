...
from mcp_client12 import current_weather, forecast, list_airlines, list_airports, tavily_search
...
def _llm_text(system : str,prompt : str):
    response= llm.invoke([
...
# - flight_agent: MUST select if the request mentions flights, flying,
#   airfare, airlines, airports, routes, departure, arrival,
#   direct flight, layover, overnight flight, or no overnight flights.

# - hotel_agent: MUST select if the request mentions hotels,
#   accommodation, stay, hostel, resort, lodging, or budget hotels.

# - weather_agent: MUST select if weather, climate, season,
#   temperature, rain, forecast, or weather-based packing is requested
#   or clearly useful for the trip.

# - budget_agent: MUST select if budget, affordability, cost,
#   price, spending limit, or budget constraints are mentioned.

# - itinerary_agent: MUST select for trip planning, vacation planning,
#   day-by-day planning, sightseeing, activities, or places to visit.

# IMPORTANT AGENT SELECTION RULES:

# 1. If the user mentions any flight-related requirement,
#    ALWAYS select flight_agent.

# 2. "No overnight flights" is a flight-related requirement.
#    Therefore, ALWAYS select flight_agent when the user mentions
#    no overnight flights or similar flight-time preferences.

# 3. If the user mentions budget hotels or accommodation,
#    ALWAYS select hotel_agent.

# 4. If the user gives a spending limit such as
#    "under 2 lakh", "under ₹2 lakh", "$2000 budget", etc.,
#    ALWAYS select budget_agent.

# 5. For a travel planning request, ALWAYS select itinerary_agent.

# 6. Select only the agents that are actually relevant to the request.

# 7. Do not reject an agent just because the user did not provide
#    complete information. The specialist agent can handle missing
#    details using the available tools and data.
...
# Extract the user's travel constraints accurately.
...
def supervisor_agent(state:TravelState):
    query = state['user_query']
    prompt = f'''
You are an supervisor of a real world multiagent travel Planning system.

Decide which specialist agents needed for this user request.
...
"""
    ...
    result = _llm_text('You are an expert flight planning specialist.',prompt)
